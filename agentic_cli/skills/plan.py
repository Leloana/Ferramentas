...
## Verification
- [ ] (id:V) <how we know the goal is met — typically a command to run or files to inspect>

Rules for tasks:
- Each task must mention SPECIFIC file paths from the project.
- Each task must be doable in 1-3 tool calls.
- Each task must have a verb (create, modify, replace, add, remove, run).
...
- IDs are integers, starting at 1.
- Use 3-8 tasks. Group tiny related actions into one task.
...
Rules:
1. Use ONLY the tools listed in your prompt.
2. Output tool calls as ```json {"tool": "...", "args": {...}} ``` blocks.
3. One tool per turn. Wait for the result before the next.
4. When the task is done, output a final line:
     SUMMARY: <one-line description of what you changed>
   then stop. Do NOT call any more tools.
5. If you cannot complete the task with the tools you have, output:
     FAILED: <reason>
   and stop.
6. PowerShell syntax for run_command.
7. Modifying existing file → patch_file. New file → write_file."""


VERIFIER_SYS = """You are an adversarial verifier. You have READ-ONLY tools.

You receive a completed plan (with checkboxes) and the list of files that
executors claimed to touch. Your job: confirm or deny.

Protocol:
1. For each task marked [x], call read_file on the relevant files and
   confirm the described change is actually present.
2. Default to "incomplete" — only confirm what you have personally seen.
3. Tasks marked [✗] or [⊘] are already known-failed; just acknowledge them.
4. End with EXACTLY this JSON block as your final output:

```json
{
  "verdict": "ok" | "incomplete" | "broken",
  "confirmed": [<task ids>],
  "rejected": [{"id": N, "reason": "..."}],
  "summary": "<one paragraph>"
}
```

Be skeptical. A task marked [x] that you cannot verify in the files is
"rejected"."""
...
def _build_executor_user_msg(task: Task, plan_md: str, goal: str,
    ...
    return f"""GOAL OF THE OVERALL PLAN:
{goal}
...
def run(args, ctx):
    ...
    console.print(Panel(
        f"executing {total} tasks (dependency order)",
    ...
    for idx, task in enumerate(ordered, start=1):
        ...
        executor = SubAgent(
            ...
            tools_whitelist=task.tools or None,
            num_ctx=8192, label=f"executor {idx}/{total} (id={task.id})",
            max_tool_calls=15,
        )
